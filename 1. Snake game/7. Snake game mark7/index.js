function execute(){
    ...
    if (todo[0]=="ArrowRight" && stop==0){
        console.log('ArrowRight pressed so going right')
        stop=setInterval(goRight, fps)
    }
    else if (todo[0]=="ArrowLeft" && stop==0){
        console.log('ArrowLeft pressed so going left')
        stop=setInterval(goLeft, fps)
    }
    else if (todo[0]=="ArrowDown" && stop==0){
        console.log('ArrowDown pressed so going Down')
        stop=setInterval(goDown, fps)
    }
    else if (todo[0]=="ArrowUp" && stop==0){
        console.log('ArrowUp pressed so going Up')
...
function goRight(){
    ...
    if (i>=25){
        ...
        if (todo.length>0){
            ...
            execute()
            console.log('execute goRight closed')
        }
    }
}
function goLeft(){
...
addEventListener('keydown',e=>{
    ...
    console.log(todo)
    execute()
    console.log('execute eventListener closed')
})